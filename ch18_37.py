...
def calculate():                    # 執行計算並顯示結果
    #result = eval(equ.get())       # 原本的CODE  不能執行 12+001 15+02等等
    result = eval(re.sub(r'([+,\-,*,/,%])0*([1-9])', r'\1\2', equ.get())) # 改成這樣就可以了
    # ([1-9])是group 1 ，\1 代表 group 1，就是把第一部分用 +group 1 取代。
    equ.set(equ.get() + "=\n" + str(result))
...
def show(buttonString):             # 更新顯示區的計算公式
    ...
    if content == "0":
        content = ""
    content1 = re.sub(r'(\w*)([+,\-,*,/,%])0*', r'\1\2', content)
    equ.set(content1 + buttonString)
